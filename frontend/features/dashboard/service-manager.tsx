...
export function ServiceManager() {
  ...
  const query = useQuery({
    ...
    queryFn: () => listAllServices(token as string),
  ...
  const create = useMutation({
    mutationFn: (input: {
      ...
      price_cents: number;
    }) => createService(token as string, input),
    onSuccess: () => {
      invalidate();
      setFormOpen(false);
    },
  ...
  const toggle = useMutation({
    mutationFn: (input: { id: string; is_active: boolean }) =>
      updateService(token as string, input.id, {
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="Los servicios inactivos dejan de ofrecerse en la reserva."
        title="Servicios"
      />
        ...
        <ul className="divide-y divide-line">
          {query.data.map((service) => (
            ...
            >
              ...
              <Button
                isLoading={
                  toggle.isPending && toggle.variables?.id === service.id
                }
                onClick={() =>
                  toggle.mutate({
                    id: service.id,
